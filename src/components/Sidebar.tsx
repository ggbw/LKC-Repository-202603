...
const roleIcon: Record<string, string> = {
  ...
  parent: "fas fa-users",
  hod: "fas fa-chart-line",
  hoy: "fas fa-chart-pie",
};
    ...
    <div className="w-[230px] flex-shrink-0 flex flex-col overflow-y-auto" style={{ background: "#1a3fa0" }}>
      <div className="px-4 pt-4 pb-3 border-b" style={{ borderColor: "rgba(255,255,255,0.15)" }}>
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="text-[13px] font-bold" style={{ color: "#e6edf3" }}>
              LKC School
            </div>
            <div className="text-[10px] mt-px" style={{ color: "#484f58" }}>
              Management System
            </div>
          </div>
        </div>
        ...
        {items.map((item, i) => {
          if (item.sec)
            return (
              <div
                key={i}
                className="px-4 pt-3 pb-1 text-[9px] font-semibold uppercase tracking-wider"
      ...
      <div className="px-4 py-3" style={{ borderTop: "1px solid rgba(255,255,255,0.15)" }}>
        <div className="flex items-center gap-2 mb-2">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[11px] font-semibold truncate" style={{ color: "#ffffff" }}>
              {profile?.full_name || "User"}
            </div>
            <div className="text-[9px] truncate" style={{ color: "rgba(255,255,255,0.5)" }}>
              {profile?.email}
            </div>
          </div>
        </div>
