...
import { useAssignments, useSubmissions, useSubjects, useTeachers, useSubjectTeachers, useStudents, useStudentSubjects, useInvalidate } from '@/hooks/useSupabaseData';
  ...
  return (
    <div className="page-animate">
      <div className="flex justify-between items-center mb-4">
        <div><div className="text-lg font-bold"><i className="fas fa-tasks mr-2" />Assignments</div><div className="text-[11px]" style={{ color: 'hsl(var(--text2))' }}>{visibleAssignments.length} total</div></div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-[12.5px]">
            <thead><tr style={{ background: 'hsl(var(--surface2))', borderBottom: '2px solid hsl(var(--border))' }}>
              {['Title','Subject','Form','Due Date','Type','Status','Submissions', ...((isAdmin || isTeacher) ? ['Actions'] : [])].map(h => (
              ...
              {visibleAssignments.map((a: any) => {
                ...
                return (
                  <tr key={a.id} className="hover:bg-[hsl(var(--surface2))] cursor-pointer" style={{ borderBottom: '1px solid #f6f8fa' }}
                    ...
                    <td className="py-2.5 px-3.5 font-semibold" style={{ color: '#1a3fa0' }}>{a.title}</td>
                    ...
                    <td className="py-2.5 px-3.5"><Badge status={a.submission_type || 'file'} /></td>
                      ...
                      <td className="py-2.5 px-3.5">
                        <div className="flex gap-1">
                          {a.state === 'draft' && (a.teacher_id === myTeacher?.id || isAdmin) && (
                            <Btn variant="primary" size="sm" onClick={async (e: any) => {
                              e.stopPropagation();
      ...
      {modal && <AssignmentModal subjects={subjects} teachers={teachers} subjectTeachers={subjectTeachers}
        onClose={() => { setModal(false); invalidate(['assignments']); }} />}
    </div>
  );
}
...
function AssignmentDetail({ id, onBack }: { id: string; onBack: () => void }) {
  ...
  return (
    <div className="page-animate">
      ...
      <div className="flex items-start gap-4 mb-5 pb-4" style={{ borderBottom: '1px solid hsl(var(--border))' }}>
        ...
        <div className="flex-1">
          ...
          <div className="flex gap-2 items-center mt-1.5">
            ...
            <span className="text-[11px]" style={{ color: 'hsl(var(--text2))' }}>{a.subjects?.name} · {a.form}</span>
          </div>
        ...
        {isStudent && a.state === 'published' && !mySubmission && (
          <Btn onClick={() => setSubmitModal(true)}><i className="fas fa-paper-plane mr-1" />Submit</Btn>
        )}
      </div>
      ...
      {isStudent && mySubmission && (
        <Card title="My Submission" className="mb-3.5">
          <div className="text-[12px]" style={{ color: 'hsl(var(--text2))' }}>
            <div>Status: <Badge status={mySubmission.status || 'submitted'} /></div>
            {mySubmission.submission_text && <div className="mt-2 whitespace-pre-wrap">{mySubmission.submission_text}</div>}
            {mySubmission.obtained_marks !== null && <div className="mt-2 font-bold">Marks: {mySubmission.obtained_marks}/{a.total_marks}</div>}
      ...
      <Card title={`Submissions (${submissions.length})`}>
        {submissions.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-[12.5px]">
              <thead><tr style={{ background: 'hsl(var(--surface2))', borderBottom: '2px solid hsl(var(--border))' }}>
                {['Student','Submitted','Type','Marks','Status', ...((isTeacher || isAdmin) ? ['Grade'] : [])].map(h => (
              ...
              <tbody>
                {submissions.map((s: any) => (
                  <tr key={s.id} style={{ borderBottom: '1px solid #f6f8fa' }}>
                    ...
                    {(isTeacher || isAdmin) && (
                      <td className="py-2.5 px-3.5">
                        <GradeSubmissionBtn submission={s} totalMarks={a.total_marks} onDone={() => invalidate(['submissions'])} />
                      </td>
                    )}
                  </tr>
                ))}
      ...
      {submitModal && <SubmitAssignmentModal assignment={a} studentId={myStudent?.id} onClose={() => { setSubmitModal(false); invalidate(['submissions']); }} />}
    </div>
  );
}

function GradeSubmissionBtn({ submission, totalMarks, onDone }: { submission: any; totalMarks: number; onDone: () => void }) {
  const { showToast } = useApp();
  ...
  if (!grading) return <Btn variant="outline" size="sm" onClick={() => setGrading(true)}><i className="fas fa-pen mr-1" />Grade</Btn>;

  return (
    <div className="flex gap-1 items-center">
      <input type="number" value={marks} onChange={e => setMarks(e.target.value)} className="w-14 border rounded py-0.5 px-1 text-[11px] font-mono"
        style={{ borderColor: 'hsl(var(--border))' }} placeholder="Marks" />
      <input type="text" value={comment} onChange={e => setComment(e.target.value)} className="w-24 border rounded py-0.5 px-1 text-[11px]"
        style={{ borderColor: 'hsl(var(--border))' }} placeholder="Comment" />
      <Btn variant="primary" size="sm" onClick={async () => {
        await supabase.from('submissions').update({
          obtained_marks: marks ? Number(marks) : null, teacher_comment: comment || null, status: 'graded',
        }).eq('id', submission.id);
        showToast('Graded');
        setGrading(false);
        onDone();
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <ModalFoot>
        ...
        <Btn onClick={save} disabled={saving}>{saving ? 'Submitting…' : 'Submit'}</Btn>
      </ModalFoot>
    </Modal>
  );
}

function AssignmentModal({ subjects, teachers, subjectTeachers, onClose }: { subjects: any[]; teachers: any[]; subjectTeachers: any[]; onClose: () => void }) {
  const { showToast } = useApp();
  ...
  const save = async () => {
    ...
    const { error } = await supabase.from('assignments').insert({
      ...
      teacher_id: myTeacher?.id || teachers[0]?.id, state: 'draft',
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <ModalBody>
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <Field label="Submission Type">
            <FieldSelect value={submissionType} onChange={setSubmissionType} options={[
              { value: 'file', label: 'Softcopy' }, { value: 'text', label: 'Hardcopy' }, { value: 'both', label: 'Both' },
            ]} />
          </Field>
        </div>
      ...
      <ModalFoot>
        <Btn variant="outline" onClick={onClose}>Cancel</Btn>
        <Btn onClick={save} disabled={saving}>{saving ? 'Saving…' : 'Create Draft'}</Btn>
      </ModalFoot>
    </Modal>
  );
}
