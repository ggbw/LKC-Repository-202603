...
export default function HODReportPage() {
  ...
  // Only include results from exams marked for report card
  ...
  const departments = useMemo(
    () => [...new Set(teachers.map((t: any) => t.department).filter(Boolean))].sort(),
    [teachers],
  );
  ...
  const analysis = useMemo(() => {
    const filtered = reportResults.filter((r: any) => {
    ...
    filtered.forEach((r: any) => {
      ...
      if (!byForm[studentForm][key]) {
        ...
        GRADES.forEach((g) => {
          byForm[studentForm][key].grades[g] = 0;
        });
      }
    ...
    return byForm;
  }, [reportResults, examName, dept, teachers]);
    ...
    <div className="page-animate">
      <div className="mb-4">
        ...
        <div className="text-[11px]" style={{ color: "hsl(var(--text2))" }}>
          Teacher to Teacher Analysis · Pass% = 50% · Credit Pass = 60%+
        </div>
      </div>
            ...
            <table className="w-full border-collapse" style={{ fontSize: "11.5px" }}>
              <thead>
                <tr style={{ background: "#f6f8fa", borderBottom: "2px solid hsl(var(--border))" }}>
                  ...
                  <th
                    className="py-[9px] px-3.5 text-center text-[10px] font-semibold uppercase"
                    style={{ color: "hsl(var(--text2))" }}
                  >
                    Form
                  </th>
              ...
              <tbody>
                {formOrder.map((form) => {
                  ...
                  GRADES.forEach((g) => {
                    tot[g] = rows.reduce((s, r) => s + (r.grades[g] || 0), 0);
                  });
                  const fT = GRADES.reduce((s, g) => s + tot[g], 0);
                  const fC = fT ? Math.round((["A*", "A", "B", "C"].reduce((s, g) => s + tot[g], 0) / fT) * 100) : 0;
                  const fP = fT
                    ? Math.round((["A*", "A", "B", "C", "D"].reduce((s, g) => s + tot[g], 0) / fT) * 100)
                    : 0;

                  return (
                    <React.Fragment key={form}>
                      <tr style={{ background: "#f0f6ff" }}>
                        <td colSpan={2} className="py-2 px-3.5 font-bold text-[11px]" style={{ color: "#1f6feb" }}>
                          📚 {form}
                        </td>
                        {GRADES.map((g) => (
                          <td key={g} />
                      ...
                      {rows.map((row, ri) => {
                        ...
                        const credit = total
                          ? Math.round(
                              (["A*", "A", "B", "C"].reduce((s, g) => s + (row.grades[g] || 0), 0) / total) * 100,
                            )
                          : 0;
                        const pass = total
                          ? Math.round(
                              (["A*", "A", "B", "C", "D"].reduce((s, g) => s + (row.grades[g] || 0), 0) / total) * 100,
                            )
                          : 0;
                        return (
                          <tr key={ri}>
                            ...
                            {GRADES.map((g) => (
                              <td
                                ...
                                style={{ color: (row.grades[g] || 0) > 0 ? "hsl(var(--text))" : "#d0d7de" }}
                              >
                                {row.grades[g] || 0}
                              </td>
                            ))}
                            ...
                            <td
                              ...
                              style={{ color: credit >= 60 ? "#1a7f37" : "#cf222e" }}
                            >
                              {credit}%
                            </td>
                            <td className="text-center font-bold" style={{ color: pass >= 50 ? "#1a7f37" : "#cf222e" }}>
                              {pass}%
                            </td>
                          </tr>
                        );
                      ...
                      <tr style={{ background: "#f0fff4" }}>
                        <td colSpan={2} className="py-2 px-3.5 font-bold text-[11px]" style={{ color: "#1a7f37" }}>
                          TOTAL
                        </td>
                        {GRADES.map((g) => (
                          <td key={g} className="text-center font-bold font-mono" style={{ color: "#1a7f37" }}>
                            {tot[g]}
                          </td>
                        ))}
                        <td className="text-center font-bold font-mono" style={{ color: "#1a7f37" }}>
                          {fT}
                        </td>
                        <td className="text-center font-bold" style={{ color: fC >= 60 ? "#1a7f37" : "#cf222e" }}>
                          {fC}%
                        </td>
                        <td className="text-center font-bold" style={{ color: fP >= 50 ? "#1a7f37" : "#cf222e" }}>
                          {fP}%
                        </td>
                      </tr>
                      <tr>
                        <td colSpan={GRADES.length + 5} style={{ height: "8px" }} />
                      </tr>
                    </React.Fragment>
                  );
                })}
